...
int c_in;                                //乘客状态：0为下电梯或不存在，1为等待电梯，2为在电梯上
...
int cin_request(int a)
{
    int b;
    printf("Passenger from the %d floor, please enter your motivation below.\n",a);
    scanf(("%d"),&b);
    return b;
}
int main()
{
    printf("Please enter one floor with passenger waiting on per line, and enter a '0' at the end.\n");
    for(int i=0;i<=1;i++)
        for(int j=1;j<=10;j++)
            human[j][i]=0;
    while(1)
    ...
    while(empty()==0)//
    {
        ...
        {
            print=1;
            human[level][1]=cin_request(level);
            human[level][0]=2;
            load++;
        }
        switch (level){
